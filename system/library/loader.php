...
class Loader {
	...
	public function model(string $route): void {
		...
		if (!$this->registry->has('model_' . str_replace('/', '_', $route))) {
			...
			$class = 'WebSiteToYou\\' . $this->config->get('application') . '\Model\\' . str_replace(['_', '/'], ['', '\\'], ucwords($route, '_/'));

			if (class_exists($class)) {
				$proxy = new \WebSiteToYou\System\Library\Proxy();
	...
	public function library(string $route, array &...$args): object {
		...
		$class = 'WebSiteToYou\System\Library\\' . str_replace(['_', '/'], ['', '\\'], ucwords($route, '_/'));
	 ...
	 * @param    string $route
	 */
	public function extension(string $route): void {
		$route = preg_replace('/[^a-zA-Z0-9_\/]/', '', $route);

                
		if (!str_starts_with($route, ' ')) {
			$file = DIR_EXTENSION .'analytics/' .$route . '.php';
		} else {
		...
		return function (mixed &...$args) use ($route) {
			...
			if ($result) {
				...
				if (!$this->registry->has($key)) {
					...
					$class = 'WebSiteToYou\\' . $this->config->get('application') . '\Model\\' . str_replace(['_', '/'], ['', '\\'], ucwords($key, '_/'));
